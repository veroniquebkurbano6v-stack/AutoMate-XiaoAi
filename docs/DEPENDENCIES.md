...
| 屏幕视觉描述 VLM（主） | GLM-5.3-Flash     | 智谱 AI    | OpenAI 兼容 API           | 是            | 回退 GUI-Plus 描述 |
| 通用视觉理解 VLM    | Qwen3-VL-Flash    | 阿里云百炼    | DashScope OpenAI 兼容 API | 是（可复用百炼 Key） | 视觉描述降级         |
...
| androidx.security（security-crypto） | 本地配置与敏感信息加密存储 | Apache-2.0         |

> 完整 Gradle 依赖以 `app/build.gradle.kts` 为准。
> 早期版本曾集成 RapidOCR 端侧 OCR 通道，已在后续迭代中移除（改用 VLM 视觉描述 + GUI-Plus 坐标直出，减少误判与资源占用）。
...
### 3.2 后处理流程（本项目如何生成 545 条 SOP）

App 内置知识库（`app/src/main/assets/kb/sop_raw/`，545 条 JSON）为 CAGUI **后处理结果**，流程如下：
...
5. **清洗与质控**：去重、剔除无效条目、人工抽检修正，形成最终 545 条离线 SOP。
...
| 检索     | 向量(task 0.7 + keyword 0.3) + 关键词 → RRF 融合(RRF\_K=60)，置信度分档（high≥0.55 / medium≥0.45 / low），low 档被工具层过滤 |
| 评测     | 32 场景检索命中率 100%（top-1 / top-3）                                                                        |

> **数量说明**：原始 CAGUI 轨迹经后处理后为 586 条候选，其中部分条目**操作路径完全一致、
> 仅个别实体（如具体店名/人名/商品名）不同**，综合考量予以合并删去，后续迭代中又补充了
> 政务服务等新领域条目，最终保留 **545 条**。
