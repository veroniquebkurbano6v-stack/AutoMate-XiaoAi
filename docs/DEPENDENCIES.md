...
3. **隐私与数据流向合规**（如实说明本地/云端边界）：
   - **完全本地（不出手机）**：端侧知识库（545 条 SOP + bge-small-zh ONNX 嵌入）与检索、聊天记录持久化（Room）——本地运行、不上传；
   - **云端处理（上云）**：任务执行所需的**屏幕截图会上传国产云端模型**（每轮屏幕视觉描述/点击定位走智谱 GLM、阿里百炼 GUI-Plus/Qwen3-VL，文本决策走 DeepSeek），云端返回描述/坐标/决策后由本机执行；截图仅用于当轮任务推理，按各平台服务协议处理，**数据不出境**（均为国产云）；
   - **不自部署声明**：本项目不自行部署 LLM/VLM（学生团队资源与预算限制），架构为「端侧执行 + 国产云端推理」；云端 Key 由用户在 App「设置」页自行填写，不随源码/APK 分发。
4. **代码合规**：第三方库均采用宽松/开源协议（见第二部分），无闭源依赖除商业 API 之外的情形。
5. **授权边界**：App 使用无障碍服务仅用于本地任务执行与屏幕理解，不做后台采集，不读取联系人等无关敏感信息；涉及支付等
   不可逆操作时需用户确认（`supervised` 机制），必要时 request_user_action 交接用户亲自执行。
6. **专业决策边界**：小艾为「代用户操作手机」的 GUI 助手，不提供医疗、法律、金融等专业建议与决策；
   涉及医疗挂号、缴费、理财等场景时仅代执行操作流程，**不替代专业诊断/判断，最终结果需用户自行核实**；
   建议老年/视障用户在监护人指导下开通并使用关键功能。
